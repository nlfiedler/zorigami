...
** TODO [#A] Preventing mistakes
*** validate that the dataset basepath exists and is readable
**** add a validate function to Dataset, returns Error if any problem
**** GraphQL mutation to return an error message or something
**** web interface to check the return value and display an error message
...
*** default to run once every 30 days
...
**** look for =dbase/= records which are of type =Pack=
**** same retention validation: if the database Pack upload_time is older than Store retention then delete it
**** delete pack file from pack store
**** delete =dbase/= record from database
** TODO [#C] SolidJS
