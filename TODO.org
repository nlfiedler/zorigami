...
**** TODO use case: enqueue new file/tree restore request
**** TODO use case: query pending/completed requests
***** completed requests include error messages, if any
**** TODO use case: cancel restore requests
**** TODO database restore use case changes
***** check if restore has any pending requests, error out if non-empty
**** TODO remove the =put_back= use case and GraphQL endpoint
**** TODO remove the =restore_file()= function
*** frontend
**** TODO write a usecase for submitting the tree/file digests
**** TODO write a usecase for checking the progress of the request
**** TODO write a usecase for canceling a pending request
**** TODO direct user to a "restore requests" page that shows progress
**** TODO write a new bloc for this restore management
**** TODO consider how to show errors with respect to files
**** TODO maybe show list of all files being restored, with status/error in a table
