* Tasks
** DONE Test full backup and restore on ~yubaba~ using ~minio~
*** DONE clear minio packs
*** DONE replicate =zeniba/shared= from ~chihiro~
*** DONE delete database files in =/zeniba/zorigami/database=
*** DONE deploy zorigami
*** DONE create the dataset, set to ~manual~, start a backup
*** DONE wait for backup to complete, check for errors; no errors
*** DONE destroy/create dataset, fix ownership and permissions
*** using the one snapshot [18/18]
- [X] Antonia
- [X] Applications
- [X] Artwork
- [X] Books
...
- [X] tanuki
- [X] Writings
** DONE restore the missing files by finding the real packs and fixing the metadata
*** file information
:path: tanuki/assets/blobstore/2023/05/21/2215/01h106dakr177a8c105etsr3fw.jpg
:digest: e390da5ca7b3bcb53de5201915b2c617a2cef419c8660772f835651bb452fb0f
:pack: 781ec6d07a4bdec46c506be1f4b765eab4824b93d325766021e9d28e1684998d
** TODO fix restore for symlinks
*** error message =error restoring link= ... =No such file or directory (os error 2)=
...
** TODO Run =replica.sh= from ~chihiro~ to ~kohaku~ again
** TODO Rebuild and deploy to ~chihiro~, destroy existing packs and start again
** TODO Rebuild and deploy on ~tampopo~, destroy existing packs and start again
** Database Integrity
...
*** fix the file name extensions to be consistent (=.salt=, =.nacl=, =.tar=, etc)
*** find all =create_dir_all= and add a =.context("")= to them
...
*** should clean up dataset workspaces on statup and periodically
...
*** The buttons to start and stop the backup are not obvious at all
*** The whole display of snapshots needs to be improved
*** Stop using the list item view thing and the leading/trailing widgets
*** Design and build a custom widget in which the buttons actually look like buttons
