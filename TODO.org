...
*** TODO write tests for backup and restore of symbolic links
**** create a symlink as a part of a data set
**** backup the entire data set
**** modify the symlink contents
**** backup the entire data set
**** request to restore the tree containing the sym link
**** currently will fail because restore does not handle symlinks at all
...
**** base64 encode the file contents and store in the tree
**** base64 encoded file of 64 bytes becomes 88 bytes
**** gzip of the 88 bytes turns into 113 bytes (including original file name)
**** create a new =TreeReference= named =SMALL=
