...
*** TODO why are the backups so large? each snapshot is hundreds of MB
**** about 500,000 files are backed up
**** why is the database backup over 600 MB in size?
***** kohaku backup is much smaller, only 40 MB with 146,000 files
***** probably the number of tree records
**** need a query that shows the counts for all record types
...
**** need a way to show differences between any two snapshots
