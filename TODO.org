...
*** TODO backend
**** =process.rs= needs to consider start time in the time range (if any)
...
** Put Back feature
*** DONE write usecase for ~put back~ that restores file to original path
*** DONE add GraphQL mutation to invoke ~put back~ usecase
*** DONE Rename the "restore" button to "download"
*** DONE Add a "put back" button to invoke the GraphQL mutation
**** DONE write data source function to run graphql mutation
**** DONE test the data source function
**** DONE expose repository function to restore file
**** DONE test the restore function in repo
**** DONE write a new client-side usecase to restore a file
**** DONE decide if =TreeBrowserBloc= will handle the restore, or a new bloc
**** DONE can show each file name as it is restored using a =SnackBar=
**** DONE test the new bloc event/state
*** DONE should be able to put back all selected files
*** DONE deselect files once they are restored
** Cloud Backup
...
** Loose WebUI tasks
*** TODO use =CircularProgressIndicator= instead of a "loading" text widget
*** TODO move =usecases/usecase.dart= to be under =domain=
...
**** DONE add padding around the form, its touching the edge of the screen
