...
*** TODO snapshots page [0/8]
- [ ] show dataset base path somewhere
- [ ] show list of snapshots with pagination
- [ ] maybe use =bulma-list= to show snapshot date/time, with more info on right
- [ ] add =/tree= route to show tree by its digest
- [ ] show breadcrumbs of the path leading up to this tree
- [ ] breadcrumbs would be built up via clicking
- [ ] jumping directly to a tree route would not show breadcrumbs
...
*** TODO deployment [0/1]
- [ ] update Dockerfile
*** TODO documentation [2/9]
- [ ] how to generate SDL file (=env GENERATE_SDL=public/schema.graphql cargo run=)
- [ ] how to generate GraphQL TypeScript code (=bun run codegen=)
...
*** TODO delete temp database and refine initial setup experience
...
** TODO Prevent deleting a pack store that is still being used by a data set
...
** TODO rename SFTP property =remote_addr= to simply =address=
