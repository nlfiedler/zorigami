...
**** TODO try using cargo workspaces to speed up compilation
***** c.f. https://doc.rust-lang.org/book/ch14-03-cargo-workspaces.html
***** domain, data, and preso become library crates
***** enforces no dependencies from domain to other layers (crate dependencies are acyclic)
***** =main.rs= moves to binary crate named =server=
***** each unit compiles separately
***** tests move into the appropriate crate
***** can run all tests or tests for one crate (=cargo test -p <name>=)
