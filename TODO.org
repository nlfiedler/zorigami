...
***** TODO write test for the local store implementation
...
***** TODO =TreeEntry.reference= should be a sum type to represent link, tree, file
****** should probably not be an =Option= either
***** TODO seems like checksums and digests should be a struct
****** TODO implement =TryFrom= for =&str=
****** TODO derive =Eq= trait
****** TODO what about ordering?
***** TODO find differences between snapshots
***** TODO write integ tests for =take_snapshot()=
****** TODO with xattrs, if supported
****** TODO with symlinks
****** TODO with nested directories
****** TODO with files changing to dirs, links, vice versa
***** TODO open database instance and pass =DB= around
***** TODO produce pack files from changes
***** TODO restore files from backup
***** TODO should struct fields be =pub= or not and have accessors?
***** DONE insert xattr records for each tree entry (see =process_path()=)
**** TODO Look for calls to =unwrap= that maybe should be converted to errors
e.g. path.file_name().ok_or_else(|| err_msg("invalid file path"))?;
**** TODO Chunk serde should skip digest and database must set digest using key (sans =chunk/= prefix)
