* Tasks
** TODO restore database from nothing (simplified version)
*** only restore the database with a matching computer UUID
*** user defines a temporary pack configuration to start
*** retrieve the most recent database archive from the pack store
*** extract the archive to =.backup= directory
*** restore the database from =.backup= directory
**** need to close the current database, delete its files
**** call the restore database function
**** open the newly restored database
*** write a use case and test with mocks
*** write the flutter interface to invoke this
** TODO update to latest rusoto release
** TODO update to latest serde release
*** the =serde::export= module is now private
*** may need to use serde in the domain layer entities
*** maybe consider an entirely different serde solution
