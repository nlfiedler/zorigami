...
*** zip uses AES encryption (not AES-256)
*** do _not_ use 7z encryption since using a salt is more secure
*** DONE try https://crates.io/crates/sevenz-rust for pack files
*** DONE try https://crates.io/crates/sevenz-rust for database backup
*** DONE remove =tar= and =flate2= dependencies
*** DONE doc: update =NOTES.md= regarding pack file and database snapshot formats
*** DONE fork sevenz-rust and fix issue #9
...
*** before (using gzip compressed chunks tar file)
...
*** any number of containers, any number of blobs
*** prices similar to Amazon S3 and Google Cloud Storage
*** container names conform to same rules as S3 and Google
*** blob naming rules are very generous
*** configuration parameters must include ~account~ name
*** c.f. https://learn.microsoft.com/en-us/azure/storage/blobs/blob-upload-function-trigger-javascript
