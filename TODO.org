* Tasks
** Cleanup and Testing
*** TODO try removing the =AddNestedTypenameVisitor= hack from the dart code
*** TODO make abstract version of ~backup~ manager code for testing
**** rename =process.rs= to =schedule.rs= to make clear it schedules (backups)
**** just like the =Restorer= trait and implementation
**** make the =perform_backup()= function module private
**** use the mock version of backup manager in =process.rs= to test the actor logic
**** add tests for =start_backup= to ensure repeatable behavior
