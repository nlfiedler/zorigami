...
*** TODO what happens to files that have an error during backup?
...
*** TODO backup where the only new file is zero bytes in length
**** backup code chokes on the non-existent pack file
