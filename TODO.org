...
**** DONE Put the new logic in a module called =supervisor=
**** DONE use =@hourly= and =@daily= for simple backup schedules
**** DONE add the cron expression to the =Dataset= type
**** DONE modify the GraphQL resolvers to validate the cron expression
**** DONE spawn a thread in =main= to run the supervisor
**** DONE fetch datasets and find those that should run
**** DONE spawn a thread to backup each available dataset
**** DONE add logging to allow for debugging the supervisor
** GraphQL and WebUI
...
**** TODO Put the bucklescript output into =lib/js=
