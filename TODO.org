...
*** DONE Get the redux state management working
**** DONE choose a state management crate
***** https://crates.io/crates/reducer (MIT)
***** https://crates.io/crates/redux-rs (MIT)
***** https://github.com/SergioBenitez/state (MIT/Apache)
**** DONE set up a singleton store instance using =lazy_static=
**** DONE implement getter methods for =State=
**** DONE register listeners with the Display thing
**** DONE the =Reactor= impl for =Display= calls listeners with state
**** DONE add a fn that takes a callback to receive current state
**** DONE store tracks progress of backup process
**** DONE engine emits actions to update progress
**** DONE state holds overall data for tracking progress
**** DONE write tests for engine related behavior
*** TODO Logging of backup process
**** main process spawns thread to listen for state events
**** Write to log using the =log= crate (how =main= does it now)
...
***** check if dataset is already running (it will be in redux store)
***** spawn a thread to run the backup for that dataset
***** backup thread inserts dataset into redux store at start
***** backup thread removes dataset from redux store upon completion
