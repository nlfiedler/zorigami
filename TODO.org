...
*** some files are missing from the packs and cannot be restored
*** missing files found so far (snapshot 7/31/2023):
...
*** missing ~tanuki~ resource error:
#+begin_src
process_tree: error processing file tanuki/assets/blobstore/2023/05/21/2215/01h106dakr177a8c105etsr3fw.jpg: missing file: SHA256("e390da5ca7b3bcb53de5201915b2c617a2cef419c8660772f835651bb452fb0f")
#+end_src
*** missing ~Quicken Data~ file error:
#+begin_src
Antonia/Downloads/Quicken Data/Quicken Data.qdfm/Contents/Quicken State/DRAT#15967 Fixed: No such file or directory (os error 2)
#+end_src
** Database Integrity
...
**** files missing from a pack file means the file record must be deleted so it can be saved again
...
**** perhaps record files only after they are packed and uploaded
**** perhaps record file status as enum of =SCANNED=, =PACKED=, and =UPLOADED=
**** perhaps scan/backup needs to consider if file was actually saved properly previously
**** if existing file record is present, ensure the chunk/pack records also exist
**** if a single pack upload fails, the entire snapshot is discarded
...
*** most recent restore request is not shown in the list
*** copy button next to checksum in tree display to copy digest to clipboard
...
*** add ~manual~ frequency that turns off automatic backup
