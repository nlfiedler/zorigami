...
#+END_SRC
** Clean Architecture
*** Consider how to change the implementation to model Clean Architecture
...
**** THIRD: migrate of the rest of the code
**** Data
***** pack stores become data sources for packs
***** pack repository would provide logic for handling multiple stores
****** =store_pack()=
****** =retrieve_pack()=
**** Presentation
...
**** Remove all dangling tree records
**** Remove all dangling file records
**** Remove all dangling chunk records
**** Mark all dangling pack records as ready for removal
...
*** Consider if the database schema needs to change for easier pruning
**** How to scan for dangling trees and files without exploding memory?
**** Mark and Sweep but with database records: will be rather slow
***** maybe scan and update in large batches to avoid frequent access
**** Read about the Garbage-First garbage collector in Java
