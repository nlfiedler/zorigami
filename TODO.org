...
*** DONE =store_amazon= must use DynamoDB for database bucket renames
*** TODO set up local server for testing with full source tree
...
**** =zip= crate does not (yet) support encryption
**** check on file size limits (original zip has 4gb limits, zip64 is much higher)
*** DONE evaluate how =sevenz-rust= handles the password
**** uses the coder salt and padding to hash password
*** =sevenz-rust= does not yet write with encryption
...
*** TODO compute before and after metrics for comparison
*** TODO remove =tar= and =flate2= dependencies
*** TODO maybe can remove =sodiumoxide= crate
*** TODO doc: update =NOTES.md= regarding pack file format
** Chunking
