...
** Flutter Action Plan
*** DONE set up dev environment with flutter web
*** DONE get something on the screen (in chrome?)
*** DONE add graphql client support
*** DONE query configuration details and show in a panel
*** cookbooks reference: https://flutter.dev/docs/cookbook
*** DONE read about BLoC and state management
**** c.f. https://www.didierboelens.com/2018/08/reactive-programming---streams---bloc/
**** c.f. https://bloclibrary.dev/
**** c.f. https://pub.dev/packages/bloc
**** c.f. http://reactivex.io (and RxDart)
**** c.f. https://pub.dev/packages/rxdart
**** DONE VS Code extension: =FelixAngelov.bloc=
*** DONE read about flutter code organziation
**** c.f. https://medium.com/flutter-community/flutter-code-organization-revised-b09ad5cef7f6
**** study https://github.com/boeledi/Streams-Block-Reactive-Programming-in-Flutter
**** =api/= for remote calls
**** =blocs/= for all of the blocs
**** =models/= for data objects
**** =pages/= for page definitions
**** =widgets/= for shared widgets
*** DONE read about clean architecture and code layout
...
**** DONE part 13
**** DONE part 14
**** DONE https://resocoder.com/2019/10/26/flutter-bloc-library-tutorial-1-0-0-stable-reactive-state-management/
*** DONE look at https://pub.dev/packages/dartz
**** maybe copy their =Either= and make our own =Result=
**** code is coupled with the entire library, difficult to extract one piece
**** code is likely falling out of maintenance
**** code has many Dart-2 transition issues, not as good as it was before
*** DONE look for a dart package license checker: there is no up-to-date package
*** TODO reorganize these notes
**** capture the information in one section
***** widgets use presentation logic holder (BLoC), do not interact with domain/data layers
**** the actionable items need to be ordered by Clean layers: domain, data, preso
***** preso
****** add =flutter_bloc= and =equatable= packages
****** use =BlocBuilder= to react to changes in the Bloc state
****** use =BlocListener= to listen for state changes, e.g. display an error in a snackbar
****** find a service locator for dependency injection
*** TODO design the architecture using the "clean" architecture
**** TODO decide on what the "features" are going to be
***** Android has dynamic feature packages that are separately downloaded modules
****** c.f. https://developer.android.com/guide/app-bundle/dynamic-delivery
****** e.g. account mgmt, product search, listing items for sale, processing payments
***** the entire application is essentially one "feature", backup
***** or are the features instead the stores, datasets, snapshots, etc?
***** or maybe the screens?
**** list of functionality that needs to fit somewhere
1) define entities
2) display entities
3) edit entities
4) delete entities
5) browse snapshots
6) restore files
7) display snapshot status
*** TODO write basic models, use cases, etc, and tests
**** start from the inside out: domain, then data, then preso
**** copy https://github.com/LiewLi/result and make it better, more Rust-like
...
**** create a "core" feature that holds base classes (e.g. base class for =UseCase=)
**** use the Dart callable classes feature for use cases (see part 3 of tdd-clean-architecture)
**** dependency injection
***** look for "service locators"
***** tdd clean tutorial suggests https://pub.dev/packages/get_it
*** TODO using clean architecture likely means not using =graphql_flutter= at all
**** instead the data layer would use the =graphql= package, like mujina does
**** look for the "future builder" and devise patterns for the "loading", "error", and "ready" states
**** or copy whatever =graphql_flutter= is doing
**** tdd clean tutorial encodes the remote data states in the bloc state itself (c.f. part 10)
*** presentation layer testing
**** use Bloc to receive events and invoke the use cases, yielding updated state
***** there will be a Bloc for each of the domain entities and their use cases
**** use https://pub.dev/packages/bloc_test and https://pub.dev/packages/mockito
*** TODO document this decision to use the clear architecture, include references
...
****** the "presentation logic holders" is where BLoC sits (within the presentation layer)
****** organizing the features and core components
******* he has a "core" and a "features" directory which has the nested features
******* let's instead have the features and "core" at the same level
***** flutter package: https://pub.dev/packages/flutter_clean_architecture
****** has no concept of features
****** splits presentation layer into "app" and "device"
****** comes with a bunch of interfaces and such around MVC
*** TODO look at other backup programs interface design
