...
*** DONE move datasets to their own page
*** DONE add a navbar link for the datasets route
*** DONE browse snapshots via webui
**** DONE show list of datasets by computer id, basepath, and schedule
**** DONE if a dataset has a latest snapshot with an end time, that row can be clicked
**** DONE clicking on dataset will display information about the snapshot
*** DONE browse directory tree via webui
**** DONE write GetTreeQuery to fetch the tree
**** DONE build a Tree component that displays a given tree
**** DONE embed tree component in Snapshot and display the initial tree
**** DONE clicking on tree entry will open that tree
**** DONE when opening a tree push current tree onto a stack
**** DONE when stack is non-empty, first row has a button to pop the stack
*** TODO clicking on file entry will display file details somewhere
**** tree will likely be vertically unlimited, so maybe next to the tree?
**** or appear over the tree in a modal?
...
** Loose issues
*** TODO apparently the backup is including the =.tmp= entry from the dataset
**** putting the =.tmp= in the dataset is probably a bad idea
**** at the very least, need to exclude that from the tree scan
...
*** TODO the tooltips on the navbar are not showing
