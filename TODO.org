...
**** currently hard-coded to =localhost:8080= in the code
**** using a relative endpoint is not supported by the package
**** maybe there is a browser/flutter-web function for getting the current address
**** may have to do the old solution of the build stuffing the value into the code
**** maybe there is a way to inject build-time settings (a la rust)
*** TODO change =main.rs= to serve up the flutter interface
**** use =flutter build web= and see where the output goes
**** change the paths in =main.rs= to serve up that compiled content
**** probably start cleaning up the files in =public=
...
*** TODO need a solution for =openssl= crate that will work on Windows
