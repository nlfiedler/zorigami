...
*** TODO currently when running locally the backup thread panics
**** add more logging in the error cases
*** TODO ensure backup progress is logged to the console
*** TODO consider again if schema should strip store key prefix
*** TODO use this to replace =replicaz= for local backup
**** deploy in the same manner as tanuki (one instance, store db on host)
...
***** it expects a store key, but user cannot see that value
