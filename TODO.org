* Tasks
** TODO check on server backup
*** DONE make sure =/safe/packs= is populated
*** DONE make sure cloud storage is populated
*** TODO see if a rename was stored in firestore
** TODO Measure thread pool improvement (~took snapshot~)
...
*** TODO add support for bucket renames using DynamoDB (https://crates.io/crates/rusoto_dynamodb)
**** TODO learn how to create a table
**** TODO learn how to insert records
**** TODO learn how to query records
*** notes about S3 and Glacier
**** ~S3 Stardard-IA~ wants ~128kb~ or larger objects, no less than ~30~ days
**** ~S3 Glacier~ wants ~128kb~ or larger objects, no less than ~90~ days
**** ~S3 Glacier~ automatically creates indices for easy listing of archives
**** ~S3 Standard - Infrequent Access~ is nearly same price as Google ~Nearline storage~
**** ~S3 Glacier Instant Retrieval~ is same price as Google ~Coldline storage~
** TODO Rewrite packing to use low-overhead format
