...
** More Functionality
*** TODO The stores should be the ones to create bucket names
**** Allows Glacier and S3 stores to re-use buckets as necessary
**** Other stores would just use =generate_bucket_name()= as usual
**** Try testing the bucket limit with Minio, should max out at 100
