...
**** DONE Need GraphQL mutations to delete stores and datasets
**** DONE Add a delete button to the store edit form
...
***** DONE will need validators for each kind of store
****** not all fields with the same name will validate the same way for different kinds of stores
****** some fields will be required and some will not, even if they have the same name
***** DONE lack of exposed types in Formality makes a generic form difficult
...
***** TODO need to parameterize new store form somehow to avoid code duplication
***** TODO make a different form and validation module for each type of store
***** TODO need to encode the options into a single "options" field for GraphQL
