...
*** TODO restores page [0/3]
- [ ] show list of restore requests
- [ ] add button to cancel a request
- [ ] provide method for restoring a database
*** TODO deployment [0/1]
- [ ] update Dockerfile
...
*** TODO delete temp database and refine initial setup experience [0/4]
- [ ] when there are no stores, say so, prominently show ~add~ button
- [ ] when there are no datasets, say so, prominently show ~add~ button
- [ ] when there are no snapshots, indicate that something will happen eventually
- [ ] when there are no stores, ~Restore~ page should suggest going to stores page to define some stores
*** TODO test restoring the database from a backup
**** delete the =database= directory but not the =packs=
**** home page should guide toward defining a pack store and link to restore page
...
*** add ~Automatic~ to =Schedule= entity
*** add ~Automatic~ option to dataset schedule form in webui
*** scheduler treats ~Automatic~ schedule as ~Hourly~ in terms of running backups
*** add support for automatic pruning to =PruneSnapshots= usecase
*** if scheule is ~Automatic~ then invoke =PruneSnapshots= after every backup
*** web ui: either ~schedule~ or ~retention~ is used to enable this behavior
a) ~schedule~ is set to automatic and ~retention~ form is disabled
b) ~schedule~ can be anything and ~retention~ is set to automatic
...
*** TODO home page [0/14]
- [ ] for each dataset, card header arrow icon navigates to =/datasets= page
- [ ] for each dataset, show schedule frequency
- [ ] for each dataset, show retention policy
- [ ] for each dataset, show status (running, paused, finished, etc)
- [ ] format dates using local time if possible
- [ ] for each dataset, show how many snapshots exist
- [ ] for each dataset, show date/time of oldest snapshot
- [ ] for each dataset, show date/time of most recent snapshot
- [ ] for each dataset, provide button to prune according to policy
- [ ] for each dataset, provide button to start/stop backup
- [ ] snopshot prune button needs to show status of pruning (like upload button)
- [ ] consider how start/stop button could use an =Action= to wait on a running backup
- [ ] when there are no stores, suggest visiting stores page to start setup
- [ ] when there are no data sets, suggest visiting datasets page to start setup
...
- [ ] show status of running snapshot (no end time)
*** TODO settings page [0/3]
- [ ] fields to set the hostname and username
- [ ] show computer UUID based on inputs
- [ ] explain how these are used to generate bucket names
** Clean up
...
**** normally show the current path of the file being restored
...
** Snapshot browsing
*** moving forward/backward in time should stay in selected directory (currently resets to the top)
