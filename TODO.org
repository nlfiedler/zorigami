* Tasks
** TODO [#A] Centralize backup and restore into a single supervisor and priority queue
*** design objectives
**** app state becomes informative only, does not actually change anything
**** use cases enqueue requests into a single priority queue (higher beats lower)
**** requests and priorities
| Request      | Priority | Principle                                                    |
|--------------+----------+--------------------------------------------------------------|
| Stop backup  |       30 | must be able to stop a backup in order for restore to happen |
| Restore      |       20 | must process restore operations before doing more backups    |
| Prune        |       10 | backup cannot start until pruning is complete                |
| Start backup |        1 | when there is nothing else to do, then backup can start      |
*** TODO implementation [0/9]
- [ ] draw on paper the overall design for clarity
- [ ] find a priority queue implementation
- [ ] define request objects for start, stop, restore
- [ ] implement a single actix supervisor for everything
