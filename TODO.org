...
** Online Backup
*** TODO =PackBuilder.add_file()= should use an order map for =file_chunks=
**** that way it always processes files in the same order rather than randomly
**** use a =BTreeMap= and invoke =first_entry()= to get the first one
