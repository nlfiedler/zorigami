...
*** DONE snapshot time range shows stop time of the most recent snapshot on all snapshots
*** DONE fix all the stateful widgets to use the new convention
*** DONE snapshots display should not have > button when there are no snapshots
