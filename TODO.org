...
***** engine loads all of the stores into =BackupMaster=
***** =send_one_pack()= sends pack to each store
***** new struct for the store/bucket/object tuple: =PackLocation=
***** =store_pack()= returns =PackLocation=
***** =send_one_pack()= saves list of =PackLocation=
***** =record_completed_pack()= saves =PackLocation= list in pack record
***** =store= module provides utility functions for delegating to multiple stores
***** delegator for =retrieve_pack()= will choose most suitable pack store
