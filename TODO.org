...
*** TODO form for configuring dataset
**** TODO on home page, indicate if there are no stores defined, link to stores page
***** i.e. only show the datasets forms when there are stores defined
**** TODO add forms for adding/editing datasets to the home page
***** use the same basic logic as the stores page
**** TODO select a store for the dataset (one for now)
**** TODO set the backup schedule with simple rules (e.g. "daily")
