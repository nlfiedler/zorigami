* Tasks
** TODO [#A] Replace Flutter with Leptos
*** TODO home page
**** DONE name it ~Home~ and show summary of datasets
**** DONE for each dataset, card header arrow icon navigates to =/datasets= page
**** DONE for each dataset, show schedule frequency
**** DONE for each dataset, show retention policy
**** DONE for each dataset, show status (running, paused, finished, etc)
**** DONE format dates using local time if possible
**** DONE for each dataset, show how many snapshots exist
**** DONE for each dataset, show date/time of oldest snapshot
**** DONE for each dataset, show date/time of most recent snapshot
**** TODO for each dataset, provide button to prune according to policy
**** TODO for each dataset, provide button to start/stop backup
**** TODO snopshot prune button needs to show status of pruning (like upload button)
...
**** DONE when there are no stores, suggest visiting stores page to start setup
**** DONE when there are no data sets, suggest visiting datasets page to start setup
*** TODO snapshots page
...
*** TODO copy =doc/LEPTOS.md= from tanuki
*** TODO update =doc/HISTORY.md=
*** TODO write ADR for why Leptos (can copy from tanuki)
** Leptos improvements
*** pack stores page
**** deleting the current store needs to refresh the parent route
**** updating store label needs to refresh parent route
*** nav bar items should highlight for matching route (e.g. ~Stores~ highlights for =/stores*=)
...
** Use =tzarrays::simple::OptimalArray= instead of =Vec= in ~get_all~ functions in =EntityDataSourceImpl=
*** functions in =EntityDataSourceImpl= to be changed [/]
...
*** Learn more about Pragmatic Clean Architecture
**** c.f. https://www.milanjovanovic.tech/pragmatic-clean-architecture
**** c.f. https://arg-software.medium.com/taming-the-chaos-a-developers-guide-to-pragmatic-clean-architecture-in-net-%EF%B8%8F-c0b05de359a7
**** c.f. https://www.ssw.com.au/rules/rules-to-better-clean-architecture/
