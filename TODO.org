...
*** Use a mocking library for isolating tests
**** https://crates.io/crates/mockall
*** Organize the code into domain/data/preso layers
**** FIRST: Define the domain logic of the application
***** entities
****** checksum
******* the =checksum= functions become static functions on =Checksum= type
****** chunk
****** dataset
****** file
****** pack
...
**** SECOND: implement this design for ONE THING all the way down
***** start a new branch as this will be destructive and time-consuming
***** pick an entity and implement it
...
***** implement its use cases
***** write unit tests (using mocks for data layer) for the domain layer
***** implement the data model
****** models have a copy of every field of the corresponding entity
****** models use =#[serde(remote = "Entity")]= trick to get serde for free
***** implement the data repository
***** implement the data source
****** =database.rs= becomes a module under =data/sources=
****** most everything is serialized in the data source and stores as bytes to the =database=
***** write unit tests (using mocks for the data source) for data layer
***** write integration tests for the data sources
**** THIRD: implement the rest of the CRUD use cases
**** move management code to =domain/managers=
***** state.rs
***** supervisor.rs (rename to =process.rs=)
**** FOURTH: implement the other use cases
***** start backup
***** stop backup
***** restore file
**** Data
***** pack stores become data sources for packs
...
*** Remove either the dead code or the =#[allow(dead_code)]= pragma
