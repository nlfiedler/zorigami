...
| mini   |    65 | 336632 |       5179 |
| kohaku |       |        |            |
** DONE Set up mini backup to use Azure (=zorigamini=) and local
** DONE Test database restore from Azure
** TODO Test file restore from Azure, fails
*** trying to restore =devscripts/LICENSE= and get ~Io(Custom { kind: InvalidInput, error: "dist overflow" }, "")~
** TODO Test database restore from Amazon
*** need to run a new backup, old one has gzipped database snapshots
** TODO Test file restore from Amazon
*** need to run a new backup, old one has tar pack files
** TODO when testing azure pack store with wrong account name, ui hangs
** TODO when restoring database from azure pack store with wrong account name, hangs
** TODO web ui barfs when configured for wrong backend address
*** happens when ~environment_config~ is invoked with the wrong =BASE_URL=
** TODO need a ~refresh~ button on the snapshots listing, like file restore page
