...
***** implement =get_snapshot()= in data source for GraphQL resolver
***** implement the GraphQL =latest_snapshot()= on =Dataset=
**** TODO implement ~update data set~ usecase (and graphql mutation)
**** TODO implement ~delete data set~ usecase (and graphql mutation)
***** write data source tests for dataset put/get/delete
...
**** backup manager code
***** handle datasets that have no stores (skip them)
