...
****** just remove the =schedule= field and create a new named =schedules=
****** =Dataset.schedules= is a =Vec= of the type data type
****** add =#[serde(default)]= to the =Dataset= struct
****** define a =default()= for =Dataset= that defaults schedule to daily
***** schedule will consist of a frequency and range
****** hourly, daily, weekly, monthly
****** no range for hourly
****** daily has an hour range
****** weekly has optional days and hour range
****** monthly has optional days and hour range
***** everything is strongly typed, no string parsing/formatting
***** TODO encode this using enums in the GraphQL schema
***** TODO change GraphQL schema to use the new =schedules= field
***** TODO change web ui to provide one frequency/range combo for now
***** TODO backend will support multiple frequency/range pairs per dataset
***** TODO remove the =cron= create and its usage
