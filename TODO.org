...
** Flutter Action Plan
*** TODO debug failing index page load for production build
**** run =flutter build web= and start the server, then try to open the page
**** (cannot just open the =index.html=, CORS rejects the schema)
**** fails with syntax error (shown in the browser console)
**** stack trace includes reference to =font_collection.dart=
**** may be a font loading issue (i.e. the roboto font)
