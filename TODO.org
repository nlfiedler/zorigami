...
** TODO webui bugs and improvements
*** DONE all expandable panels have suddenly stopped expanding
*** DONE fix the file name extensions to be consistent (=.salt=, =.nacl=, =.tar=, etc)
*** DONE find all =create_dir_all= and add a =.context("")= to them
*** TODO scrolling snapshot directory with lots of entries (239) sometimes becomes unresponsive
...
*** TODO snapshot time range shows stop time of the most recent snapshot on all snapshots
*** TODO web ui should refresh restore requests page every few seconds
*** DONE fix all the stateful widgets to use the new convention
*** TODO snapshots display should not have > button when there are no snapshots
...
**** excludes: =.Trash, .android, Library, **/Downloads, **/node_modules, **/target, fvm=
