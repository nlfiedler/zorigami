* Tasks
** TODO build and run tests on Windows, fix
** TODO data set form input validation is broken for time range?
*** try to add a dataset that runs hourly, ADD button does nothing
*** commenting out the time range validator makes it pass
** TODO consider refactoring the =domain/managers= code
*** the entity-like objects should go into =domain/entities=
*** some code could become a usecase, such as the =take_snapshot()= function
**** see =~/mod.rs= and =~/take_snapshot.rs=
