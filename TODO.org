...
*** TODO use a persistent connection to speed up the process
**** store the =Session= in a =Mutex= in the =SftpStore=
**** change =connect()= to try to use existing session
**** if session was closed, make a new one
