...
**** DONE implement ~get snapshot~ usecase (and graphql query)
**** DONE implement ~get tree~ usecase (and graphql query)
**** DONE convert tests in =schema_test.rs= to unit tests
**** DONE write a store validator to validate input to use cases
**** DONE get the frontend working again
***** store query/mutation
****** new: supply type (e.g. ~local~), label, and list of name/value pairs
****** update: same as new, plus the store identifier is required
***** dataset query/mutation
****** renamed =key= field to =id=
****** added =workspace= field on input
****** =computerId= is not guaranteed to be present in query
***** ensure snapshots and browsing is working
...
**** TODO add padding around the form, its touching the edge of the screen
