...
** TODO replace pack file format with =exaf-rs=
*** remove all uses of =sodiumoxide=, it is deprecated
*** also pack the database using exaf
*** also encrypt the database
*** write an ADR to explain why this and not something else
- zip crate doesn't encrypt
- sevenz-rust had a data corruption/loss bug, and its very slow
- tar/gzip is clumsy and awkward
