...
*** validate that the dataset basepath exists and is readable
...
*** maybe make use of the =prune_extra()= in pack repository
*** database backups to be removed according to pack store retention policy
** TODO [#C] Capture background tasks issues to database
*** weekly restore test could have errors, currently only logged
*** weekly database scrub only logs error, need to surface via GraphQL/web
*** need to consider how this can be mocked and tested, log files are just kinda okay
*** write errors to an ~errors~ log file in JSON format
*** include date-time of event, the action performed (restore test, scrub, etc), error details
*** rotate the logs each week(?), retain N with timestamped names
*** GraphQL query to read contents of error logs
*** ~Home~ page to query for any errors, show a button along the top to an ~errors~ page
*** ~errors~ page is not in the navbar, only accessible from ~Home~ if there are errors
