...
***** TODO change the existing code to treat basepath as one of many options
***** TODO need a GraphQL query for the available kinds of stores
****** just return every kind of the store with default config values
