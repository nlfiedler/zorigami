...
****** =backup.rs= (nee =engine.rs=) to perform backup/restore operations
...
**** TODO copy enough =schema.rs= to preso layer to expose ~new store~ mutation
***** change GraphQL representation of =Store= to match model
****** id: String!
****** type: String!
****** label: String!
****** properties: [NamedValue!]! (where NamedValue is a name/value string pair)
**** TODO enable enough =main.rs= to expose the GraphQL endpoint
**** TODO implement ~get stores~ usecase (and graphql query)
**** TODO implement ~update store~ usecase (and graphql mutation)
**** TODO implement ~delete store~ usecase (and graphql mutation)
**** TODO implement ~new data set~ usecase (and graphql mutation)
***** change =DataSet= stores to be just the store identifier (not the "name")
**** TODO implement ~get data sets~ usecase (and graphql query)
**** TODO implement ~update data set~ usecase (and graphql mutation)
**** TODO implement ~delete data set~ usecase (and graphql mutation)
**** TODO implement ~start backup~ usecase (and graphql mutation)
***** management code (state, engine, supervisor)
***** entities for trees, snapshots, files, packs, etc
...
*** TODO Remove either the dead code or the =#[allow(dead_code)]= pragma
