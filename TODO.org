...
- [X] show icons for small files and symbolic links
- [X] empty files should show something rather than nothing
- [ ] show status of running snapshot (no end time); add envar to keep backups running
** TODO [#A] Scheduler to test restoring a random file weekly
...
*** TODO add a =restore_test()= to =Restorer= [0/3]
- [ ] find a random =file/= record from the database (may need a new db fn for performance)
- [ ] find the chunk and pack that contains the file
- [ ] retrieve the pack(s), extract, verify file digests
*** TODO add =restore_test()= to =Leader= and its supervisor to call restorer
*** TODO add envar to set the frequency for the restore test
*** expose the result somewhere that the home page ~dashboard~ and display
** TODO [#A] Scheduler to perform a ~scrub~ of the database weekly
...
*** use a hashset or bloom filter to track every tree that is visited to avoid looping
*** expose the result somewhere that the home page ~dashboard~ and display
** TODO [#B] Preventing mistakes
...
*** Add a new pack store to an existing dataset
**** Could provide a means of copying packs between stores
**** Would be like other tasks, managed by supervised actor, processes in the background
**** Would travese each bucket in the source and ensure target has everything
...
**** Like adding a new pack store, add function to ensure target has everything in source
