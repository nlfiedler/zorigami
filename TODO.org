...
*** current solution results in many buckets with a single pack file
...
*** consider how bucket reuse domain logic should interact with pack data sources
**** some data sources will have their own bucket reuse policy dictated by the provider
...
*** Similarly, prevent a backup from starting while pruning is in progress
...
**** use the =upload_time= in the =Pack= in the database snapshot record to determine age
**** use the snapshot age for conformance with remote storage deletion policies
