...
**** DONE do not serialize the =length= field of chunk model
...
***** in =record_completed_pack()= if the chunk digest matches a file digest, do not store in database
...
***** file restore needs to know that the "chunk" digest is the file digest when assembling the chunks
***** add tests to ensure correct number of file and chunk records are created
