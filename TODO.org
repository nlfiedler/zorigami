* Tasks
** TODO add instructions for measuring code coverage of tests
*** see exaf-rs for an example that uses grcov
...
*** TODO add ~Automatic~ member to snapshot retention policy enum
*** TODO add ~Automatic~ option to snapshot retention policy form in webui
*** TODO add support for automatic pruning to =PrunerImpl=
*** TODO test: consider how to write a macro to declaratively produce lots of fake snapshots
...
** TODO [#C] Scheduler to test restoring a random file weekly
*** every week, pick a random file from the =file/= set and try to restore it, compare checksum to actual file
*** scheduler will send a new type of request to the ring leader
*** restorer needs a ~noop~ mode to simply compare the checksums of the target and restored file
*** expose the result somewhere that the home page ~dashboard~ and display
** TODO [#C] Scheduler to perform a ~scrub~ of the database weekly
*** like zfs scrub, make sure no records are missing
*** add to ~pruner~ and use the same basic visitor pattern as for pruning, visiting _everything_
*** start scanning from data sets > snapshots > trees > files > chunks > packs > xattrs
*** expose the result somewhere that the home page ~dashboard~ and display
** TODO [#C] Migrate to Vite 8.0
*** https://vite.dev/blog/announcing-vite8
** TODO [#C] Preventing mistakes
...
** TODO [#C] Dynamic bucket allocation
...
** TODO [#C] Bucket collision and renaming
