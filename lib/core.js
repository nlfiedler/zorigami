...
async function packParts (parts, outfile) {
  ...
  header.writeUInt32BE(parts.length, 8)
 ...
 * directory, with the names being the SHA1 checksum of each part. If the file
 * is compressed, it will be decompressed in place.
 *
...
async function unpackPartsV1 (infd, outdir) {
  ...
  while (index < count) {
    await readBytes(infd, buffer, 0, 4, fpos)
    fpos += 4
    const partSize = buffer.readUInt32BE(0)
