...
async function packParts (parts, outfile) {
  ...
  const buffer = Buffer.allocUnsafe(262144)
  let partNumber = 0
  ...
  for (let { path, offset, length } of parts) {
    ...
    while (length > 0) {
      const { bytesRead } = await fread(infd, buffer, 0, Math.min(buffer.length, length), offset)
      length -= bytesRead
      offset += bytesRead
      const data = buffer.slice(0, bytesRead)
      fileHash.update(data)
      await writeAndHash(data, outfd, packHash)
    }
    fs.closeSync(infd)
...
/**
 * Write the data to the file and the hasher.
 *
 * @param {Buffer} data
 * @param {number} fd
 * @param {Hash} hash
 */
async function writeAndHash (data, fd, hash) {
  await fwrite(fd, data)
  hash.update(data)
}

// compressing a file...
// c.f. https://nodejs.org/dist/latest-v10.x/docs/api/stream.html
//
// const fs = require('fs');
